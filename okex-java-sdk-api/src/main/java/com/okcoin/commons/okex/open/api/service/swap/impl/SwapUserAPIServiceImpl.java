...
public class SwapUserAPIServiceImpl implements SwapUserAPIServive {
    ...
    //单个合约持仓信息
    @Override
    public Map<String,Object> getPosition(String instrument_id) {
    ...
    //某个币种合约账户信息
    @Override
    public JSONObject selectAccount(String instrument_id) {
        return client.executeSync(api.selectAccount(instrument_id));
    }

    //获取某个合约的用户配置
    @Override
    public JSONObject selectContractSettings(String instrument_id) {
        return client.executeSync(api.selectContractSettings(instrument_id));
    }

    //设定某个合约的杠杆
    @Override
    public JSONObject updateLevelRate(String instrument_id, LevelRateParam levelRateParam) {
        return client.executeSync(api.updateLevelRate(instrument_id, JsonUtils.convertObject(levelRateParam, LevelRateParam.class)));
    }

    //账单流水查询
    @Override
    public JSONArray getLedger(String instrument_id, String after, String before,  String limit, String type) {
        return client.executeSync(api.getLedger(instrument_id, after, before, limit,type));
    }

    //获取所有订单列表
    @Override
    public Map<String,Object> selectOrders(String instrument_id, String after, String before, String limit, String state) {
        return client.executeSync(api.selectOrders(instrument_id, after, before, limit, state));
    }

    //获取订单信息(通过order_id)
    @Override
    public JSONObject selectOrderByOrderId(String instrument_id, String order_id) {
        return client.executeSync(api.selectOrderByOrderId(instrument_id, order_id));
    }

    //获取订单信息(通过client_oid)
    @Override
    public JSONObject selectOrderByClientOid(String instrument_id, String client_oid) {
        return client.executeSync(api.selectOrderByClientOid(instrument_id,client_oid));
    }

    //获取成交明细
    @Override
    public JSONArray selectDealDetail(String order_id, String instrument_id, String after, String before, String limit) {
        return client.executeSync(api.selectDealDetail(order_id, instrument_id, after, before, limit));
    }

    //获取合约挂单冻结数量
    @Override
    public JSONObject getHolds(String instrument_id) {
